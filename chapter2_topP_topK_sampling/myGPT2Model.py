...
class MyGPT2Model(nn.Module):
    def __init__(self):
        ...
        self.h = nn.ModuleList([MyGPT2Block(self.config, self.modelParameters, layer_idx=i) for i in range(self.config.num_hidden_layers)])
        self.ln_f = nn.LayerNorm(self.embed_dim, eps=self.config.layer_norm_epsilon)
        self.lm_head = nn.Linear(self.config.n_embd, self.config.vocab_size, bias=False)

        self.sampler = Sampler(0.1, 0.2, 20)
    ...
    def forward(self, input_ids: torch.Tensor):
        ...
        # 获取多语言头, 将hidden_states转化为对下一个token预测的概率
...
class MyGPT2Block(nn.Module):
    def __init__(self, config, modelParameters, layer_idx=None):
        ...
        self.attn = MyGPT2Attention(config, modelParameters, layer_idx=layer_idx)
...
class MyGPT2Attention(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # 在这里查看哪些值被mask了
        # logger.info('scores before mask: ', scores)
        scores = scores.masked_fill(batch_mask == 0, -1e9)
        # logger.info('scores after mask: ', scores)
