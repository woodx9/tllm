...
class PrefillModel(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, speculative_text_ids: torch.Tensor):
        ...
        input_ids = self.speculatvie_verify(logits, input_ids, speculative_text_ids)

        # 由于input_ids的长度可能会变化, 因此此时kvCache的长度也会变化，需要进行截断
        # 但是由于这个例子中kvCache没有用到，所以这里不做处理

        return input_ids, kvCache
    
    def speculatvie_verify(self, logits, input_ids, speculative_text_ids):
        # speculative_text_ids
        # Get logits for the last token in the input sequence
        logger.info('sequence_length: ', input_ids.shape)
        logger.info('speculative_text_ids: ', speculative_text_ids.shape)
        

        speculative_length = speculative_text_ids.size(1)
        last_tokens_logits = logits[:, -1 * (speculative_length + 1):, :]

        if speculative_text_ids.size(1) == 0:
            # 插入一个新的token
            new_token_id = torch.argmax(last_tokens_logits, dim=-1)
            new_token_id = new_token_id[:, -1].unsqueeze(1)
            input_ids = torch.cat((input_ids, new_token_id), dim=1)
            return input_ids

        # Apply softmax to convert logits to probabilities
        probabilities = torch.softmax(last_tokens_logits.squeeze(), dim=-1)


        top_k_values, top_k_indices = torch.topk(probabilities, k=100, dim=-1)

        orginal_input_ids_length = input_ids.size(1) - speculative_text_ids.size(1)

        last_index = 0

        for idx in range(speculative_length):
            logger.info('idx: ', idx)
            last_index = idx
            current_input_ids_index = orginal_input_ids_length + idx

            target_token_id = speculative_text_ids[0][idx]
            is_target_token_in_top_k = target_token_id in top_k_indices[idx]
            

            # 查看候选token的逻辑
            tokenizer = GPT2Tokenizer.from_pretrained('gpt2')

            target_token = tokenizer.decode(target_token_id, skip_special_tokens=True)
            logger.info('target_token:', target_token)

            candidate_opts = tokenizer.decode(top_k_indices[idx], skip_special_tokens=True)
            logger.info('candidate_opts:', candidate_opts)

            logger.info('is_target_token_in_top_k: ', is_target_token_in_top_k)
            # 查看候选token的逻辑

            if not is_target_token_in_top_k: 
                break

        if input_ids.size(1) == current_input_ids_index + 1:
            # 插入一个新的token
            new_token_id = top_k_indices[last_index + 1, 0]
            new_token_id = torch.tensor([[new_token_id]], device=self.device1)
            input_ids = torch.cat((input_ids, new_token_id), dim=1)
            
        else :
            input_ids[0][current_input_ids_index] = top_k_indices[last_index, 0]
