...
def main():
    ...
    with torch.no_grad():
        # 第一次通过调用prefillModel 来获取kvCache
        infer_token_start_time = time.time()
        result, kvCache = prefilfModel(input_ids)
        ...
        for i in range(new_token_number - 1):          

            infer_token_start_time = time.time()
            result, kvCache = decodeModel(input_ids, kvCache)
            infer_per_token_time.append((time.time() - infer_token_start_time))
