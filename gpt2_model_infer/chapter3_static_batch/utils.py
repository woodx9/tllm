...
def add_new_input_ids(tokenizer, input_ids, attention_mask, new_input_ids):
    new_input_id_column = torch.full((input_ids.size(0), 1), tokenizer.pad_token_id)
    new_attn_mask_column = torch.full((attention_mask.size(0), 1), 0)
    ...
    last_positions = np.argmax(attention_mask == 0, axis=1)
