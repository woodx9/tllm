...
class PrefillModel(nn.Module):
    def __init__(self, device1=DEFAULT_DEVICE, device2=DEFAULT_DEVICE):
        ...
        self.h = nn.ModuleList([PrefillGPT2Block(self.config, self.modelParameters, layer_idx=i, device=self.device) for i in range(self.config.num_hidden_layers)])
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor):
        ...
        logger.info('inputs_embeds:', position_ids.shape)
...
class PrefillGPT2Block(nn.Module):
    def __init__(self, config, modelParameters, layer_idx=None, device=DEFAULT_DEVICE):
        ...
        self.attn = PrefillGPT2Attention(config, modelParameters, layer_idx=self.layer_idx, device=self.device)
...
class PrefillGPT2Attention(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # 扩展为多个批次, 需要考虑上num_heads
        batch_mask = mask.unsqueeze(0).unsqueeze(0).expand(batch_size, self.num_heads, -1, -1)
