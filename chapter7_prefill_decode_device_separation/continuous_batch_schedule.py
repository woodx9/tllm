...
class ContinuousBatchSchedule:
    def __init__(self, batch_size, max_context_token_num, stop_words):
        ...
        device2 = get_device()
        self.intermediateDevice = get_device()

        self.prefillModel = PrefillModel(device1, self.intermediateDevice)
        self.decodeModel = DecodeModel(device2, self.intermediateDevice)
    ...
    def getIntermediateDevice(self):
        return self.intermediateDevice
    ...
    def encodePromptsWihSeqLen(self, prompts , seq_len):
        ...
        input_ids = encodeBatch['input_ids'].to(self.getIntermediateDevice())
        ...
        input_id_padding = torch.full((input_ids.size(0), padding_len), self.tokenizer.pad_token_id, device=self.getIntermediateDevice())
        input_ids = torch.cat((input_ids, input_id_padding), dim=-1)

        attention_mask = encodeBatch['attention_mask'].to(self.getIntermediateDevice())
        attention_mask_padding = torch.full((attention_mask.size(0), padding_len), 0, device=self.getIntermediateDevice())
    ...
    def kv_cache_AddPadding(self, kv, seq_len):
        ...
        key_padding = torch.full((batch_size, padding_len, embed_dim), 0, device=self.getIntermediateDevice())
        value_padding = torch.full((batch_size, padding_len, embed_dim), 0, device=self.getIntermediateDevice())
    ...
    def continuous_batch(self, first_batch_prompts, left_prompts):
        ...
        input_ids, attention_mask = add_new_input_ids(self.tokenizer, input_ids, attention_mask, new_input_ids, device=self.getIntermediateDevice())
        ...
        while input_ids.size(0) > 0:
            ...
            if len(left_prompts) > 0  and len(stop_decode_indexs) > 0:
                ...
                if (seq_len  > prefill_input_ids.size(1)):
                    ...
                    new_input_ids, new_attention_mask = add_new_input_ids(self.tokenizer, prefill_input_ids, prefill_attention_mask, new_prefill_input_ids, device=self.getIntermediateDevice())
